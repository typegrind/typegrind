...
void OpNewExprHandler::run(const clang::ast_matchers::MatchFinder::MatchResult& result) {
  const clang::ExplicitCastExpr* castExpr =
      result.Nodes.getNodeAs<clang::ExplicitCastExpr>("castExpr");
  ...
  if (castExpr) {
    ...
    if (!ptr->isInstantiationDependentType() && !ptr->hasUnnamedOrLocalType()) {
      handleSpecializedType(castExpr->getTypeInfoAsWritten()->getTypeLoc().getType(),
                            startLoc.getRawEncoding(), CONVERT_TO_POINTEE);
    }
  }

  if (!processingLocation(startLoc)) return;

  MacroAdder newLoggerMacro(funDecl->getNameInfo().getName().getAsString() == "operator new[]"
  ...
  if (castExpr) {
    ...
    addTypeInformationParameters(newLoggerMacro, srcType, startLoc.getRawEncoding(),
                                 CONVERT_TO_POINTEE);
  ...
  newLoggerMacro.endBuffer() << ", ";
  newExpr->getArg(0)->printPretty(newLoggerMacro.endBuffer(), nullptr,
