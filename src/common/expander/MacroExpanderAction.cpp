...
/// RewriteMacrosInInput - Implement -rewrite-macros mode.
    void expandMacrosInInput(Preprocessor &PP, std::unordered_map<std::string, std::string> &expandedFiles) {
      ...
      while (PPTok.isNot(tok::eof)) {
        ...
        if(PPLoc1.isMacroID())
        {
          ...
          if (PPLoc1.isMacroID())
            lastMacro = PP.getImmediateMacroName(PPLoc1);
          //if (lastMacro == "PAR") {
      ...
      for (auto it = Rewrite.buffer_begin() ; it != end; ++it)
      {
        ...
        if(Entry && Entry->isValid())
        {
          ...
          expandedFiles.insert(std::pair<std::string, std::string>(originalFileName,  std::string(it->second.begin(), it->second.end())));
        }
      }
...
namespace typegrind
{
  ...
  MacroExpanderAction::MacroExpanderAction(clang::tooling::ClangTool &targetTool) : targetTool(targetTool)
  {
  }

  bool MacroExpanderAction::ParseArgs (const clang::CompilerInstance &CI, const std::vector< std::string > &arg)
  {
    return isCppFile(CI);
  }
  ...
  std::unique_ptr<clang::ASTConsumer> MacroExpanderAction::CreateASTConsumer(clang::CompilerInstance &CI, llvm::StringRef InFile)
  {
    mCI = &CI;
    expand();
    return std::unique_ptr<clang::ASTConsumer>();
