...
class SpecializationHandler {
 public:
  ...
  virtual void handleSpecializedType(clang::QualType const& typeInfo, unsigned specificUniqId,
                                     PointeeConversion convertToPointee = KEEP_ORIGINAL_TYPE) = 0;
};

class BaseExprHandler : public clang::ast_matchers::MatchFinder::MatchCallback,
                        public SpecializationHandler {
 public:
  ...
  void handleSpecializedType(clang::QualType const& typeInfo, unsigned specificUniqId,
                             PointeeConversion convertToPointee = KEEP_ORIGINAL_TYPE) override;
 ...
 protected:
  ...
  static bool isReallyDependentType(clang::QualType const& typeInfo,
                                    PointeeConversion convertToPointee = KEEP_ORIGINAL_TYPE);
  void addTypeInformationParameters(MacroAdder& macroAdder, clang::QualType const& typeInfo,
                                    unsigned specificUniqId,
                                    PointeeConversion convertToPointee = KEEP_ORIGINAL_TYPE);
