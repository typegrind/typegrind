...
void DeleteExprHandler::run(const clang::ast_matchers::MatchFinder::MatchResult& result) {
  // Adds: TYPEGRIND_LOG_DELETE(_ARRAY)(locationStr, typeStr, canonicalStr,
  // (deleteExpr))
  // Or:   TYPEGRIND_EXPAND_DECLTYPE(locationStr, TYPEGRIND_LOG_DELETE(_ARRAY),
  // positionEncoding, (deleteExpr))
  const clang::CXXDeleteExpr* deleteExpr =
      result.Nodes.getNodeAs<clang::CXXDeleteExpr>("deleteStmt");
  ...
  std::string deleteMacroName(deleteExpr->isArrayForm() ? "TYPEGRIND_LOG_DELETE_ARRAY"
                                                        : "TYPEGRIND_LOG_DELETE");

  MacroAdder deleteLoggerMacro(isDependentType ? "TYPEGRIND_EXPAND_DECLTYPE" : deleteMacroName,
