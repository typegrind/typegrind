...
RecordDeclHandler::RecordDeclHandler(clang::Rewriter*& rewriter,
...
void RecordDeclHandler::run(const clang::ast_matchers::MatchFinder::MatchResult& result) {
  ...
  std::string name =
      decl->isExternCContext() ? "TYPEGRIND_RECORD_APPEND_C" : "TYPEGRIND_RECORD_APPEND";

  MacroAdder recordEndMacroAdder(name, insertPosition, insertPosition, mRewriter);
