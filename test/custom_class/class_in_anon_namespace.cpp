...
int main(void)
{
  // CHECK: C* pT = TYPEGRIND_LOG_NEW("{{.*}}/clang-typegrind/test/custom_class/class_in_anon_namespace.cpp:11", "class (anonymous namespace)::C", "class (anonymous namespace)::C", sizeof(C), new C());
  C* pT = new C();
  return 0;
}
